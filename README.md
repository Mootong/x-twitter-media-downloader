...
5. 登录 `https://x.com`，打开目标用户页面后点击扩展图标，右侧边栏会自动填入用户名。
...
- 使用可随页面导航保持打开的 Chrome 右侧边栏，不再使用临时弹出框。
- 打开边栏时自动从当前 X 用户页或帖子地址识别用户名。
- 指定用户名。
- 保存最近使用的用户名并支持下拉选择。
- 指定开始和结束日期（均包含当天）。
- 自动打开目标用户的媒体页面。
...
- 预览页可按图片或视频过滤，并在同一媒体信息列显示图片分辨率或视频码率。
- 跟踪每个选中项目的真实下载状态，并在全部完成后显示成功或失败汇总。
